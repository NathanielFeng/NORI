...
public:
    ...
    /** \brief Uniformly sample position on the surface
     * \param sampler
     *      Random generator
     * \param[out] p
     *      Sampled position on surface of mesh
     * \param[out] n
     *      The interpolated suface normal at p
    */
    void sampleSurface(Sampler *sampler, Point3f &p, Normal3f &n);

    float getPDF() { return m_dpdf->getNormalization(); }

    float surfaceArea() { return m_area; }
...
protected:
    ...
    DiscretePDF  *m_dpdf = nullptr;         ///< dpdf of the mesh
    BoundingBox3f m_bbox;                   ///< Bounding box of the mesh
    float         m_area;                   ///< Surface Total Area
};
