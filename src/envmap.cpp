...
public:
    EnvironmentMapEmitter(const PropertyList &propList) {
    ...
    Color3f bilinearInterpolation(float dx21, float dy21, Color3f Q11, Color3f Q21, Color3f Q12, Color3f Q22,
                                float dx01, float dy01, float dx20, float dy20) const
    {
    ...
    Color3f sample(EmitterQueryRecord &lRec, Sampler *sampler) const
    {
        ...
        sample1D(0, pmarginal, cmarginal, point.x(), u, pdfu);
        sample1D(u, mpdf, mcdf, point.y(), v, pdfv);
        Point2f pixel = Point2f(u, v);
        Vector3f w = pixelToDirection(pixel);
        // set the lRec parameters
        lRec.wi = -w;
        lRec.shadowRay = Ray3f(lRec.p, w, Epsilon, 100000);
...
NORI_REGISTER_CLASS(EnvironmentMapEmitter, "envmap");
NORI_NAMESPACE_END